...
function Career({ start, end, title, content, index }: Props) {
  return (
    <>
      <div
        className={`full items-center mb-[-60px] ${index % 2 === 1 && 'justify-end'} hidden lg:flex`}
      >
        ...
        >
          ...
          <h1 className="font-semibold sm:text-lg text-text dark:text-white">
            {title}
          </h1>
          {content.split('\n').map((l) => (
            <p
              key={`${title}-${l.substring(0, 10)}-${l.length}`}
              className="text-base text-text dark:text-dark-text mr-2"
            >
              {l}
            </p>
          ))}
      ...
      <div className="full items-center my-4 flex lg:hidden pl-10">
        <div
          className="
          transform rotate-3/4 left-3 text-bg-neumo dark:text-dark-neumo text-[40px] absolute z-50"
        >
          ▲
        </div>
